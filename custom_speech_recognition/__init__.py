...
class Recognizer(AudioSource):
    ...
    def recognize_lex(self, audio_data, bot_name, bot_alias, user_id, content_type="audio/l16; rate=16000; channels=1", access_key_id=None, secret_access_key=None, region=None):
        ...
        assert isinstance(
            content_type, str), "``content_type`` must be a string"
        ...
        assert region is None or isinstance(
            region, str), "``region`` must be a string"
                ...
                with urllib.request.urlopen(transcript_uri) as json_data:
                    ...
                    try:
                        transcribe.delete_transcription_job(
                            TranscriptionJobName=job_name)  # cleanup
    ...
    def recognize_tensorflow(self, audio_data, tensor_graph='tensorflow-data/conv_actions_frozen.pb', tensor_label='tensorflow-data/conv_actions_labels.txt'):
        ...
        assert isinstance(audio_data, AudioData), "Data must be audio data"
        assert isinstance(
            tensor_graph, str), "``tensor_graph`` must be a string"
        assert isinstance(
            tensor_label, str), "``tensor_label`` must be a string"
        ...
        if not (tensor_graph == self.lasttfgraph):
            ...
            self.tflabels = [line.rstrip()
                             for line in tf.gfile.GFile(tensor_label)]
        ...
        with tf.Session() as sess:
            ...
            predictions, = sess.run(
                softmax_tensor, {input_layer_name: wav_data})
